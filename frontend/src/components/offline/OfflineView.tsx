...
export function OfflineView() {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // Load cache info + offline diagnoses
  // ───────────────────────────────────────────────────────────────────────────

  useEffect(() => {
    try {
      const cachedAtRaw = localStorage.getItem(
        "cropguard-offline-cached-at"
      );

      if (cachedAtRaw) {
        setCachedAt(new Date(cachedAtRaw).toLocaleString());
      }
      ...
      if (raw) {
        ...
        // Load cached diagnoses when offline
        if (!isOnline) {
          setDiagnoses(parsed);

          addToast(
            `Loaded ${parsed.length} offline diagnoses`,
            "info"
          );
        }
      }
  ...
  const handleDownload = async (
    ...
    const interval = setInterval(() => {
      setProgress((p) => {
        ...
        return {
          ...p,
          [key]: cur + Math.random() * 12,
        };
      });
    }, 250);

    try {
      // ── Cache real diagnosis history ────────────────────────────────

      if (key === "disease-db") {
        const hist = await dashboardApi.getHistory(100);

        const diagnosesRaw = JSON.stringify(
          hist.diagnoses
        );
      ...
      // ── Cache crop guides ───────────────────────────────────────────
        ...
        addToast(
          "Crop guides cached for offline use",
          "success"
        );
      }

      // ── Simulated AI Lite Model ─────────────────────────────────────
      ...
      addToast(
        `Failed to cache ${label}. Try again.`,
        "error"
      );
    } finally {
      setTimeout(() => {
  ...
  const handleClearCache = () => {
    ...
    cacheItems.forEach((c) => {
      setCacheDownloaded(c.key, false);
    });
  ...
  const downloadedCount = cacheItems.filter(
    (c) => c.downloaded
  ).length;
    ...
    <div>
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          {downloadedCount > 0 && (
            <span className="inline-flex items-center gap-2 text-sm font-medium px-4 py-1.5 rounded-full bg-purple-100 text-purple-800">
              📦 {downloadedCount} item
              {downloadedCount > 1 ? "s" : ""} cached
            </span>
          )}
        </div>
      ...
      <Card className="p-6 mb-5">
        ...
        <div className="mt-5 p-4 bg-green-50 border border-green-200 rounded-xl text-sm text-green-700">
          💡{" "}
          <strong>
            Start with Disease Database
          </strong>{" "}
          — it caches your real diagnosis
          history so you can review past
          diagnoses without internet.
        </div>
      </Card>
          ...
          {[
            {
              ...
              ok: cacheItems[0].downloaded,
            },
            {
              label: "Read crop guides",
              ok: cacheItems[1].downloaded,
            },
            {
              label: "View past treatments",
              ok: cacheItems[0].downloaded,
            },
            {
              label: "Check prevention tips",
              ok: cacheItems[0].downloaded,
            },
            {
              label: "New AI diagnosis",
              ok: false,
            },
            {
              label: "Follow-up chatbot",
              ok: false,
            },
            {
