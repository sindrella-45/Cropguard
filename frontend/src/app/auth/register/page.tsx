    ...
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2">
      <div className="hidden md:flex flex-col justify-between bg-gradient-to-br from-green-700 to-green-900 px-12 py-12 relative overflow-hidden">
        ...
        <div className="relative z-10 mt-12">
          <h2 className="font-heading font-bold text-4xl text-white leading-snug mb-4">Start protecting your crops today.</h2>
          <p className="text-white/70 text-base leading-relaxed">Create your free account and get your first AI diagnosis in under 60 seconds.</p>
        </div>
        <div className="relative z-10 flex flex-col gap-4">
          {["Free to get started","No credit card required","Works on any smartphone","Offline mode included","Available in 5 languages"].map((f) => (
      ...
      <div className="flex items-center justify-center bg-white px-6 py-12 md:px-12">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-md">
          ...
          <h2 className="font-heading font-bold text-3xl text-gray-900 mb-1">Create your account</h2>
          <p className="text-gray-500 text-sm mb-8">Join 12,000+ farmers using CropGuard AI.</p>

          <div className="flex bg-gray-100 rounded-xl p-1 mb-7">
            <Link href="/auth/login" className="flex-1 text-center py-2 text-gray-500 text-sm font-medium no-underline hover:text-gray-700">Sign In</Link>
            <div className="flex-1 text-center py-2 rounded-lg bg-white shadow-sm text-green-700 text-sm font-semibold">Create Account</div>
          </div>

          {errors.general && (
            <div className="mb-5 p-3.5 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600">{errors.general}</div>
          )}

          <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
            <Input label="Full Name" type="text" placeholder="Your full name" value={form.name} onChange={(e) => set("name", e.target.value)} error={errors.name} />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input type={showPw ? "text" : "password"} placeholder="Min. 8 characters" value={form.password}
                  ...
                  className="w-full px-4 py-2.5 pr-11 border-2 border-gray-200 rounded-xl text-sm text-gray-800 bg-white outline-none focus:border-green-500 placeholder:text-gray-400" />
            ...
            <Input label="Confirm Password" type="password" placeholder="Repeat your password" value={form.confirm} onChange={(e) => set("confirm", e.target.value)} error={errors.confirm} />

            <label className="flex items-start gap-2.5 cursor-pointer mt-1">
              <input type="checkbox" checked={terms} onChange={(e) => setTerms(e.target.checked)} className="mt-0.5 w-3.5 h-3.5 accent-green-600 flex-shrink-0" />
            ...
            {errors.terms && <span className="text-red-500 text-xs -mt-2">{errors.terms}</span>}
